...
function M.new(opts)
  ...
  function self.tick(input)
    ...
    for index, t in ipairs(input.turbines or {}) do
      ...
      -- werden. Das ist jetzt der Normalfall -- eine eingeschwungene
      -- Turbine wird gar nicht mehr verstellt -- und spart je Takt einen
      -- Peripherieaufruf pro Turbine. Sicher ist es, weil current_flow
      -- die ZURUECKGELESENE Vorgabe ist: weicht sie je ab, wird im selben
      -- Takt wieder geschrieben.
      if tonumber(t.current_flow) == flow_decision.flow then
