...
--
-- Die Datei haelt jetzt einen Eintrag JE REAKTOR. Eine aeltere Datei mit
-- nur einem flachen Eintrag wird weiter gelesen -- ein bestehender
-- Ein-Reaktor-Knoten verliert seine Messung beim Update also nicht.

-- Gibt eine Tabelle { [reaktorname] = <roher Cache-Eintrag> } zurueck.
function M.load_units(opts)
  ...
  if type(data.units) == "table" then return data.units end
  -- Altformat: ein einzelner Eintrag ohne Einheitenkennung.
  if data.ready == true then return { unit1 = data } end
  return {}
end

-- Macht aus einem rohen Cache-Eintrag einen Zustand -- dieselben Pruefungen
-- wie M.load(), nur ohne Dateizugriff.
function M.from_cached(entry, turbine_count)
  if type(entry) ~= "table" or entry.ready ~= true
      or type(entry.max_output) ~= "number" or entry.max_output <= 0 then
    return nil
  end
  if type(turbine_count) == "number" and type(entry.turbine_count) == "number"
      and entry.turbine_count ~= turbine_count then
    return nil, "Turbinenzahl geaendert: gespeichert=" .. tostring(entry.turbine_count)
      .. " aktuell=" .. tostring(turbine_count)
  end
  local sustainable = tonumber(entry.sustainable_turbines)
  if not sustainable or sustainable <= 0 then
    return nil, "alter Cache ohne gemessene Turbinenzahl -- wird neu eingelernt"
  end
  local state = M.new_state()
  state.ready = true
  state.max_output = entry.max_output
  state.best_output = entry.max_output / (1 - M.SAFETY_MARGIN)
  state.total_turbines = entry.turbine_count
  state.sustainable_turbines = math.min(sustainable, entry.turbine_count or sustainable)
  state.reason = "LOADED_FROM_CACHE"
  return state
end

-- Schreibt ALLE Einheiten in einem Rutsch -- eine Datei, ein Schreibzugriff.
function M.save_units(entries, opts)
  opts = opts or {}
  if type(entries) ~= "table" then return false, "no entries" end
  if type(opts.path) ~= "string" or opts.path == "" then return false, "no path" end
  if type(opts.write_config) ~= "function" then return false, "no writer" end
  local out = {}
  for key, state in pairs(entries) do
    if type(state) == "table" and state.ready == true then
      out[key] = {
        ready = true,
        max_output = tonumber(state.max_output) or 0,
        turbine_count = tonumber(state.total_turbines) or 0,
  ...
  if next(out) == nil then return false, "not ready" end
