-- Builds the real reactor target list shown by the FUEL route editor.

local M = {}

function M.collect(config, fuel_status_cache)
  local list, by_id = {}, {}
  ...
  for _, route in ipairs(logistics.redstone_tree or {}) do
    if type(route) == "table" then
      add(route.reactor or route.label, route.label or route.reactor)
    end
  end
