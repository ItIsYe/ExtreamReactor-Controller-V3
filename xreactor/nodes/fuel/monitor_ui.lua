...
-- Fuel main-monitor and status-light presentation.
...
local ampel_instance = ok_ampel_mod and type(ampel_mod) == "table"
  and type(ampel_mod.new) == "function" and ampel_mod.new() or nil
...
local function get_render_target(mon, monitor_name)
  ...
  if not ok_size or type(w) ~= "number" or type(h) ~= "number" or w < 1 or h < 1 then return mon end
  local scale = monitor_scale(mon)
  if render_buffer.target and render_buffer.parent == mon and render_buffer.name == monitor_name
      and render_buffer.width == w and render_buffer.height == h and render_buffer.scale == scale
  ...
  if not ok_window or not target or type(target.setVisible) ~= "function" then reset_render_buffer(); return mon end
  render_buffer = { parent = mon, name = monitor_name, width = w, height = h, scale = scale, target = target, rebuilt = true }
  return target
end
...
local function render_frame(router, target, physical_mon, model)
  ...
  if buffered then pcall(target.setVisible, false) end
  local ok, result = pcall(router.render, router, target, model)
  if buffered then pcall(target.setVisible, true) end
...
function M.render_monitor(ctx, model)
  ...
  if not monitor_router then
    ...
    monitor_router = ctx.ui_router.new({
      ...
      monitor_name = devices.monitor_name,
      pages = {
        ...
        { name = "Router", render = function(target, current_model, should_clear)
            return ctx.get_router_ui():render(target, ctx.ui, ctx.colors, should_clear)
          end,
  ...
  elseif type(monitor_router.set_monitor_name) == "function" then
...
function M.current_page_index() return monitor_router and monitor_router.index or 1 end

function M.get_diagnostics()
  local base = monitor_router and monitor_router.get_diagnostics and monitor_router:get_diagnostics()
    or { error_count = 0, last_error = nil }
