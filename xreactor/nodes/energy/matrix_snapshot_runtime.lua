...
local function normalize_reason(reason)
  ...
  if text == "missing_method" or text == "missing method" then return "api_variant" end
  if text:find("^nil_value", 1) then return "temporary_not_ready" end
  if text:find("^call_failed:", 1) then return "temporary_read_error" end
  if text:find("^unsupported_value:", 1) then return "unsupported_value" end
...
function runtime:update_component_diag(group, adapter, metric, reason, detail)
  ...
  if previous == stamp then return end
  ...
  utils.log(self.log_prefix,
    ("Matrix component %s unavailable (%s reader=%s): reason=%s detail=%s method=%s"):format(
      tostring(metric), tostring(group.key), tostring(group.reader and group.reader.name or "n/a"),
      tostring(reason), tostring(detail), tostring(method_name or "n/a")), "WARN")
end
...
function runtime:group_key_signature(groups)
  ...
  for _, group in ipairs(groups or {}) do
    ...
    parts[#parts + 1] = table.concat({
      tostring(group.key), tostring(reader and reader.name or "n/a"), tostring(#(group.ports or {}))
    }, "|")
  end
...
function runtime:sync_group_caches(groups)
  local active = {}
  for _, group in ipairs(groups or {}) do active[tostring(group.key)] = true end
  for key in pairs(self.dynamic_cache) do if not active[key] then self.dynamic_cache[key] = nil end end
  for key in pairs(self.static_cache) do if not active[key] then self.static_cache[key] = nil end end
...
local function read_metric(group, label, fn, record_error)
  if not fn then return nil, "missing method" end
  local value, err = fn()
...
function runtime:poll_due_metrics(now_ts, groups)
  ...
  if #(groups or {}) <= 1 then per_matrix_budget = math.max(per_matrix_budget, #METRIC_ORDER) end
  ...
  for _, group in ipairs(groups or {}) do
    ...
    if type(cache) ~= "table" then cache = {}; self.dynamic_cache[cache_key] = cache end
    for _, metric in ipairs(METRIC_ORDER) do
      local getter = metric == "stored" and (adapter and adapter.getStored)
        ...
        or nil
      if getter then
        -- *_ts is the last ATTEMPT and is used only for scheduling/backoff.
        -- Freshness must be derived from *_good_ts, never from a failed read.
        ...
        if due then
          jobs[#jobs + 1] = { group = group, metric = metric, fn = getter, cache = cache, last_ms = last_duration }
        end
      end
    end
  end
  ...
  table.sort(jobs, function(a, b)
    if (a.last_ms or 0) ~= (b.last_ms or 0) then return (a.last_ms or 0) < (b.last_ms or 0) end
  ...
  local polled, poll_spent_ms = 0, 0
  local per_matrix = {}

  local function append_metric_call(group, metric, duration_ms)
    if duration_ms < 80 then return end
    diag.metric_calls[#diag.metric_calls + 1] = {
      matrix = tostring(group.key), reader = tostring(group.reader and group.reader.name or "n/a"),
      metric = tostring(metric), ms = duration_ms
    }
  ...
  for _, job in ipairs(jobs) do
    if polled >= to_poll or poll_spent_ms >= metric_time_budget_ms then break end
    ...
    if (per_matrix[matrix_key] or 0) < per_matrix_budget then
      ...
      polled = polled + 1
      if type(self.heartbeat_pump) == "function" then self.heartbeat_pump(now_ms()) end
      ...
      if value ~= nil then
        ...
        job.cache[job.metric .. "_good_ts"] = now_ts
  ...
  if #jobs > polled then
    diag.throttled = {
      due = #jobs, polled = polled, deferred = #jobs - polled,
      per_matrix_budget = per_matrix_budget, time_budget_ms = metric_time_budget_ms, spent_ms = poll_spent_ms
    }
    if self.debug_enabled and (now_ts - (self.last_throttle_log_ts or 0) >= throttle_log_interval_ms) then
      utils.log(self.log_prefix,
        ("Matrix metric polling throttled: due=%d budget=%d deferred=%d per_matrix_budget=%d time_budget_ms=%d spent_ms=%d"):format(
          #jobs, polled, #jobs - polled, per_matrix_budget, metric_time_budget_ms, poll_spent_ms))
      self.last_throttle_log_ts = now_ts
    end
  elseif self.debug_enabled and poll_spent_ms >= math.floor(metric_time_budget_ms * 0.85)
      and (now_ts - (self.last_throttle_log_ts or 0) >= throttle_log_interval_ms) then
    utils.log(self.log_prefix,
      ("Matrix metric polling near budget limit: due=%d polled=%d time_budget_ms=%d spent_ms=%d"):format(
        #jobs, polled, metric_time_budget_ms, poll_spent_ms), "WARN")
    self.last_throttle_log_ts = now_ts
  end
  return diag
end
...
function runtime:poll_static_components(now_ts, groups)
  ...
  while inspected < #ordered do
    ...
    if type(cache) ~= "table" then cache = {}; self.static_cache[key] = cache end
...
function runtime:rebuild_snapshot(now_ts, groups, diag)
  ...
  for _, group in ipairs(groups or {}) do
    ...
    local input, output = dynamic.input, dynamic.output
    local stored_ts = tonumber(dynamic.stored_good_ts) or 0
    local capacity_ts = tonumber(dynamic.capacity_good_ts) or 0
    local has_last_good = dynamic.last_good_stored ~= nil and dynamic.last_good_capacity ~= nil
    local has_complete_sample = stored_ts > 0 and capacity_ts > 0 and has_last_good
    local metric_ts = has_complete_sample and math.min(stored_ts, capacity_ts) or nil
    local freshness_ms = metric_ts and math.max(0, now_ts - metric_ts) or nil
    local missing = not has_complete_sample
    local stale = has_complete_sample and freshness_ms > stale_after_ms or false
    if missing or stale then snapshot_stale = true end

    local port_names = {}
    for _, port in ipairs(group.ports or {}) do port_names[#port_names + 1] = port.name end
    ...
    matrices[#matrices + 1] = {
      id = group.key, key = group.key, name = reader and reader.name or group.key,
      alias = label, label = label, reader = reader and reader.name or nil,
      ports = port_names, port_count = #port_names,
      stored = stored, capacity = capacity, percent = capacity > 0 and (stored / capacity) or 0,
      input = tonumber(input), output = tonumber(output), cells = tonumber(static.cells),
      providers = tonumber(static.providers), total_ports = tonumber(static.ports),
      valid = has_last_good,
      last_good_state = has_last_good and {
        ...
        input = tonumber(dynamic.last_good_input), output = tonumber(dynamic.last_good_output),
        ts = math.min(stored_ts > 0 and stored_ts or math.huge, capacity_ts > 0 and capacity_ts or math.huge)
      } or nil,
      sample_ts = metric_ts,
      freshness_ms = freshness_ms,
      missing = missing,
      stale = stale,
      status = missing and "MISSING" or (stale and "STALE"
        or ((dynamic.stored_err or dynamic.capacity_err) and "DEGRADED" or "OK"))
    }
  end
  ...
  self.last_snapshot = {
    ...
    matrices = matrices,
    total = {
      stored = total.stored, capacity = total.capacity, percent = total_percent,
...
function runtime:tick(ts)
  ...
  if self.last_snapshot and (self.last_snapshot.ts or 0) > 0
      and (now_ts - (self.last_poll_ts or 0)) < min_tick_spacing_ms then return end
...
function runtime:get_snapshot(max_age_ms)
  ...
  local stale = snapshot.stale == true or (snapshot.ts or 0) <= 0
  if max_age_ms and tonumber(max_age_ms) and tonumber(max_age_ms) > 0 then
    stale = stale or freshness > tonumber(max_age_ms)
  end
