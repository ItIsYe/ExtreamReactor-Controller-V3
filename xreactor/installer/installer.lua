...
local function detect_peripherals()
  local detected = {
    reactors = {},
    turbines = {},
    modems = {}
  ...
  for _, name in ipairs(peripheral.getNames()) do
    ...
    if peripheral_type == "BigReactors-Reactor" then
      table.insert(detected.reactors, name)
    elseif peripheral_type == "BigReactors-Turbine" then
      table.insert(detected.turbines, name)
    elseif peripheral_type == "modem" then
      table.insert(detected.modems, name)
    end
  end
  if #detected.reactors > 0 or #detected.turbines > 0 or #detected.modems > 0 then
...
local function main()
  ...
  local detected = detect_peripherals()
  local use_detected = (#detected.reactors > 0 or #detected.turbines > 0 or #detected.modems > 0)
  ...
  local wireless
  local wired
  if #detected.modems == 0 then
    print("Warning: No modems detected. Switching to manual entry.")
  end

  if use_detected and #detected.modems > 0 then
    wireless = detected.modems[1]
    if detected.modems[2] then
      wired = detected.modems[2]
    else
      print("Only one modem detected. Please provide a wired modem side.")
      wired = prompt("Wired modem side", "left")
    end
  else
    wireless = prompt("Wireless modem side", "right")
    wired = prompt("Wired modem side", "left")
  end
  ...
  elseif role == roles.RT_NODE then
    local use_detected_rt = use_detected and #detected.reactors > 0
    if not use_detected_rt then
      print("Warning: No reactors detected. Switching to manual entry.")
    end

    if use_detected_rt then
