...
local severity_rank = { CRITICAL = 1, WARN = 2, WARNING = 2, INFO = 3 }
...
local function sev_key(value)
  ...
  return s == "WARNING" and "WARN" or s
end
...
local function alert_matches(alert, state)
  ...
  if state.search ~= "" then
    ...
    local target = table.concat({
      normalize_text(alert.title), normalize_text(alert.message), normalize_text(alert.code),
      normalize_text(source.node_id), normalize_text(source.device_id)
    }, " ")
    if not target:find(state.search, 1, true) then return false end
  end
  return true
end
...
local function sort_alerts(list, mode)
  ...
  if mode == "node" then
    table.sort(list, function(a, b)
      ...
      local av = severity_rank[sev_key(a.severity)] or 99
      local bv = severity_rank[sev_key(b.severity)] or 99
  ...
  table.sort(list, function(a, b)
    local av = severity_rank[sev_key(a.severity)] or 99
    local bv = severity_rank[sev_key(b.severity)] or 99
    if av == bv then return (a.ts_last or 0) > (b.ts_last or 0) end
    return av < bv
  end)
end
...
local function build_entries(list, state)
  ...
  for _, node_id in ipairs(nodes) do
    ...
    local highest = "INFO"
    for _, alert in ipairs(group) do
      if (severity_rank[sev_key(alert.severity)] or 99) < (severity_rank[highest] or 99) then
        highest = sev_key(alert.severity)
      end
    end
...
local function fmt_time(ts)
  ...
  return os and os.date and os.date("!%H:%M:%S", n) or "--:--:--"
end

local function role_short(role)
  for _, entry in ipairs(role_filters) do if entry.key == role then return entry.label end end
...
local function render_controls(mon, state, w)
  ...
  local x = 4
  ui.text(mon, x, y + 1, "VIEW", colorset.get("muted"), colorset.get("background")); x = x + 6
  ...
  ui.text(mon, x, y + 1, "SORT", colorset.get("muted"), colorset.get("background")); x = x + 5
  ...
  badge_button(mon, state, "sort_node", x, y + 1, "NODE", state.sort_mode == "node" and "OK" or "OFFLINE")

  x = 4
  ui.text(mon, x, y + 3, "SEVERITY", colorset.get("muted"), colorset.get("background")); x = x + 9
  ...
  x = x + 2
  ui.text(mon, x, y + 3, "SCOPE", colorset.get("muted"), colorset.get("background")); x = x + 6
  ...
  badge_button(mon, state, "toggle_ack", x, y + 3, state.filters.show_acknowledged and "ACK" or "NOACK", state.filters.show_acknowledged and "OK" or "OFFLINE")

  x = 4
  ui.text(mon, x, y + 5, "ROLE", colorset.get("muted"), colorset.get("background")); x = x + 5
...
local function render(mon, model)
  ...
  model = model or {}
  local snapshot = utils.safe_serialize({ model = model, state = {
    page = state.page, selected = state.selected_id, view = state.view,
    group_mode = state.group_mode, sort_mode = state.sort_mode,
    search = state.search, search_active = state.search_active,
    filters = state.filters, collapsed = state.collapsed, mute_index = state.mute_index
  } }) or tostring(model)
  ...
  if w >= 58 then
    ...
    for i, card in ipairs(cards) do mux.metric_card(mon, 2 + (i - 1) * (cw + 1), 4, cw, 4, card) end
  else
  ...
  mux.card(mon, 2, list_top - 1, w - 3, list_height + 2, {
    ...
    status = page_status, icon = "warning"
  })

  local content_x, content_w, table_y = 4, w - 7, list_top
  if w >= 70 then
    mux.table_header(mon, content_x, table_y, content_w, {
      { label = "SEV", width = 6 }, { label = "MESSAGE", width = math.max(18, content_w - 38) },
      { label = "SCOPE", width = 8 }, { label = "NODE", width = 8 },
      { label = "ROLE", width = 6 }, { label = "TIME", width = 10 }
    })
  else
    mux.table_header(mon, content_x, table_y, content_w, {
      { label = "SEV", width = 5 }, { label = "MESSAGE", width = math.max(12, content_w - 21) },
      { label = "NODE", width = 8 }, { label = "TIME", width = 8 }
    })
  end
  ...
  for idx = start_idx, end_idx do
    ...
    if entry and row_y <= list_bottom then
      if entry.type == "group" then
        ...
        mux.data_row(mon, content_x, row_y, content_w, {
          label = string.format("%s %s (%d)", caret, entry.node_id, entry.count or 0),
          value = "", status = severity_status[entry.severity] or "LIMITED", icon = "network"
        })
      else
        local alert = entry.alert
        local source = alert.source or {}
        local sk = sev_key(alert.severity)
        local status = alert.acknowledged and "OFFLINE" or severity_status[sk] or "LIMITED"
        local title = tostring(alert.title or alert.code or source.device_id or source.node_id or "Alert")
        if alert.muted then title = "[M] " .. title end
        if alert.acknowledged then title = "[A] " .. title end
        if alert.lifecycle_state == "wieder_aufgetreten" then title = "[R] " .. title end
        local marker = alert.id == state.selected_id and ">" or " "
        local sev_label = sk == "CRITICAL" and "C" or sk:sub(1, 1)
        local line
        if w >= 70 then
          line = string.format("%s%-4s %-28s %-7s %-7s %-5s %-8s", marker, sev_label,
            mux.fit(title, 28), scope_short(alert.scope), mux.fit(source.node_id or "--", 7),
            mux.fit(role_short(source.role), 5), fmt_time(alert.ts_last or alert.ts_first))
        else
          line = string.format("%s%-3s %-22s %-7s %-8s", marker, sev_label,
            mux.fit(title, 22), mux.fit(source.node_id or "--", 7), fmt_time(alert.ts_last or alert.ts_first))
        end
        mux.data_row(mon, content_x, row_y, content_w, { label = line, value = "", status = status })
        if state.view == "active" then visible_ids[#visible_ids + 1] = alert.id end
      end
      row_y = row_y + 1
    end
  end
  ...
  state.list_bounds = {
    ...
    y1 = table_y + 2, y2 = table_y + 2 + math.max(0, end_idx - start_idx),
  ...
  if total_pages > 1 then
    ...
    state.buttons.prev = { x1 = content_x, x2 = content_x, y = list_bottom }
    state.buttons.next = { x1 = content_x + #ptxt - 1, x2 = content_x + #ptxt - 1, y = list_bottom }
  end

  local selected = find_selected(entries, state.selected_id)
  local detail_y = list_bottom + 2
  local detail_bottom = h - 1
  ...
  if w >= 68 and available_h >= 6 then
    ...
    mux.card(mon, 2, detail_y, detail_w, available_h, {
      title = "SELECTED ALERT", status = selected and severity_status[sev_key(selected.severity)] or "OFFLINE", icon = "warning"
    })
    mux.card(mon, detail_w + 3, detail_y, action_w, available_h, {
      title = "ACTIONS", status = selected and "WARNING" or "OFFLINE", icon = "config"
    })
    ...
    if ay + 2 <= detail_bottom then
      action_btn("mute_cycle", ax, ay, half, string.format("MUTE %dm", mute_minutes), selected and "WARNING" or "OFFLINE", selected ~= nil)
      action_btn("mute_rule", ax + half + 1, ay, aw - half - 1, rule_muted and "UNMUTE RULE" or "MUTE RULE", selected and "WARNING" or "OFFLINE", selected ~= nil)
    end
    if ay + 6 <= detail_bottom then
      action_btn("mute_node", ax, ay + 4, half, node_muted and "UNMUTE NODE" or "MUTE NODE", selected and "WARNING" or "OFFLINE", selected ~= nil)
      local ack_label = selected and selected.acknowledged and "ACKED" or "ACK"
      action_btn("ack", ax + half + 1, ay + 4, aw - half - 1, ack_label, can_ack and "WARNING" or "OFFLINE", can_ack)
    end
    if ay + 10 <= detail_bottom then
      action_btn("ack_visible", ax, ay + 8, half, "ACK VIS", #visible_ids > 0 and state.view == "active" and "WARNING" or "OFFLINE", #visible_ids > 0 and state.view == "active", { ids = visible_ids })
      action_btn("ack_all", ax + half + 1, ay + 8, aw - half - 1, "ACK ALL", #entries > 0 and state.view == "active" and "WARNING" or "OFFLINE", #entries > 0 and state.view == "active")
    end
  else
    mux.card(mon, 2, detail_y, w - 3, available_h, {
      title = "SELECTED ALERT", status = selected and severity_status[sev_key(selected.severity)] or "OFFLINE", icon = "warning"
    })
    if selected then
      local src = selected.source or {}
    ...
    local x = 4
    x = badge_button(mon, state, "mute_cycle", x, ay, string.format("MUTE %dm", mute_minutes), selected and "WARNING" or "OFFLINE")
...
local function hit_test(mon, x, y)
  ...
  if hit(state.buttons.ack) then
    local selected = find_selected(state.list_bounds.entries, state.selected_id)
    if selected and not selected.acknowledged and can_ack then return { type = "alert_ack", id = selected.id } end
    return nil
  end
...
local function handle_key(mon, key)
  ...
  if key == keys.enter or key == keys.space then
    local selected = find_selected(state.list_bounds.entries, state.selected_id)
    if selected and not selected.acknowledged and state.view == "active" then
      return { type = "alert_ack", id = selected.id }
    end
  end
