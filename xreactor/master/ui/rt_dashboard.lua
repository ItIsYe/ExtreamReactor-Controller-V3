...
local function render_rt_card(mon, x, y, w, rt)
  widgets.card(mon, x, y, w, 7, tostring(rt.id or "RT"), rt.status or "OFFLINE")
  ui.badge(mon, x + w - 12, y + 1, tostring(rt.state or "OFF"), rt.status or "OFFLINE")
  ui.text(mon, x + 1, y + 1, "Soll " .. tostring(rt.target or 0) .. "%", colors.get("text"), colors.get("background"))
  ui.text(mon, x + 1, y + 2, "Ist " .. tostring(rt.actual_output or rt.output or 0) .. "%", colors.get("text"), colors.get("background"))
  ui.text(mon, x + 1, y + 3, "Modus " .. tostring(rt.mode or "-"), colors.get("muted"), colors.get("background"))
  ui.text(mon, x + 1, y + 4, "Workflow " .. tostring(rt.assignment_reason or rt.assignment_state or "-"), colors.get("muted"), colors.get("background"))
  ui.progress(mon, x + 1, y + 6, w - 2, math.max(0, math.min(100, rt.actual_output or rt.output or 0)), rt.status or "OFFLINE")
end
...
local function render(mon, model)
  ...
  for i, rt in ipairs(model.rt_nodes or {}) do
    ...
    if i % 2 == 1 and i > 1 then y = y + 8 end
  ...
  for i, q in ipairs(model.queue or {}) do
    if i > 4 then break end
    rows[#rows + 1] = {
      text = string.format("%d. RT-%s -> %s", i, tostring(q.node_id or "?"), tostring(q.module_id or q.action or "step")),
      status = "LIMITED"
    }
  end
  if #rows == 0 then rows[1] = { text = "Queue leer", status = "OFFLINE" } end
  ui.list(mon, 3, h - 6, w - 4, rows, { max_rows = 4 })
end
