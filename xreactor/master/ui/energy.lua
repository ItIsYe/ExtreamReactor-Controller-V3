...
local function render(mon, model)
  ...
  ui.panel(mon, 2, 2, w - 2, 5, "Energy Summary", model.status or "OK")
  ...
  local left_w = math.max(30, math.floor((w - 5) * 0.6))
  local right_w = math.max(24, w - left_w - 4)
  local mid_h = math.max(9, math.floor((h - 16) * 0.5))
  ui.panel(mon, 2, 8, left_w, mid_h, "Matrix-/Storage-Details", "OK")
  local matrix_widths = { 7, 10, 10, 10, math.max(8, left_w - 41) }
  widgets.compact_header(mon, 3, 9, { "ID", "Fuellst", "Input", "Output", "Status" }, matrix_widths)
  local y = 10
  for i, m in ipairs(model.matrices or {}) do
    if y > (8 + mid_h - 1) then break end
  ...
  ui.panel(mon, 3 + left_w, 8, right_w, mid_h, "Ressourcen", "OK")
  ...
  widgets.stat_card(mon, 4 + left_w, 14, right_w - 2, "Water / Reprocessing", string.format("Wasser %.1f", r.water_total or 0), "Reproc " .. tostring(r.reprocessing_state or "-"), "OK")

  local support_y = 9 + mid_h
  ui.panel(mon, 2, support_y, w - 2, math.max(4, h - support_y), "Support-Nodes", "OK")
  local support_widths = { 7, 12, 10, 8, math.max(10, w - 40) }
