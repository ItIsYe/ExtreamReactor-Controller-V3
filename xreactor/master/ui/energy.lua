...
local function render(mon, model)
  ...
  local summary = widgets.panel_box(mon, 2, 2, w - 2, 7, "Energy Summary", model.status or "OK")
  ...
  local content_y = 10
  local content_h = math.max(10, h - content_y - 2)
  local left_w = math.max(38, math.floor((w - 4) * 0.60))
  ...
  local right_top_h = math.max(12, math.floor(content_h * 0.50))
  local resources = widgets.panel_box(mon, right_x, content_y, right_w, right_top_h, "Ressourcen", "OK")
  ...
  local support_y = content_y + right_top_h + 1
  local support = widgets.panel_box(mon, right_x, support_y, right_w, math.max(6, (content_y + content_h) - support_y), "Support-Nodes", "OK")
