...
local CONFIG = {
  ...
  DEFAULT_ALERT_RAISE_AFTER_S = 2.0, -- Seconds condition must hold before raising.
  DEFAULT_ALERT_CLEAR_AFTER_S = 3.0, -- Seconds condition must be OK before clearing.
  DEFAULT_ALERT_COOLDOWN_S = 6.0, -- Seconds between repeated alert updates.
  DEFAULT_COMMS_DOWN_WARN_SECS = 2, -- Seconds before comms-down warns.
  DEFAULT_COMMS_DOWN_CRIT_SECS = 12, -- Seconds before comms-down escalates to CRITICAL.
  ...
  DEFAULT_ALERT_MUTE_DEFAULT_MINUTES = 10, -- Default mute duration (minutes).
  DEFAULT_ALERT_MUTE_DURATIONS = { 5, 15, 30, 60 }, -- Preset mute durations (minutes).
  DEFAULT_ALERT_LOG_MUTED_EVENTS = true, -- Log muted alerts to history.
  DEFAULT_ALERT_STATE_PATH = "/xreactor/config/alerts_state.lua", -- Persistent mute state file.
  DEFAULT_ALERT_NODE_TOP_N = 3, -- Top N local alerts shipped to nodes.
  ...
  DEFAULT_DEBUG_LOGGING = true -- Enable debug logging to /xreactor_logs/master.log.
}
...
return {
  ...
  alert_debounce_s = CONFIG.DEFAULT_ALERT_RAISE_AFTER_S, -- Legacy alias.
  alert_clear_s = CONFIG.DEFAULT_ALERT_CLEAR_AFTER_S, -- Legacy alias.
