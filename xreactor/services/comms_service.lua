...
local function sanitize_channels(config)
  ...
  if type(config.channels.control) ~= "number" then config.channels.control = constants.channels.CONTROL end
  if type(config.channels.status) ~= "number" then config.channels.status = constants.channels.STATUS end
end
...
function comms_service:trace_master_rx(message)
  ...
  local key = tostring(node_id) .. ":" .. tostring(message.type) .. ":" .. tostring(message.role or "-")
  ...
  utils.log(self.log_prefix,
    ("RX diag node=%s type=%s sender=%s role=%s payload_role=%s meta_role=%s keys=%d mode=%s state=%s output=%s turbines=%s reactors=%s modules=%s"):format(
      ...
      tostring(message.role or "-"), tostring(payload.role or "-"), tostring(meta_role or "-"), count_keys(payload),
      yesno(payload.mode ~= nil), yesno(payload.state ~= nil), yesno(payload.output ~= nil),
      yesno(type(payload.turbines) == "table"), yesno(type(payload.reactors) == "table"),
      yesno(type(payload.modules) == "table")), "INFO")
end
...
function comms_service:handle_event(event)
  ...
  if event[1] == "modem_message" then
    ...
    if utils.handle_remote_log_message and utils.handle_remote_log_message(message) then return true end
