...
function router.paginate(list, per_page, page)
  ...
  return {
    page = current,
    total = total,
    start_index = (current - 1) * per + 1,
    end_index = math.min(#list, current * per)
  }
end
...
function router.new(mon_or_opts, opts)
  ...
  local self = {
    ...
    footer = { prev = nil, next = nil, indicator = nil },
    list_controls = nil,
...
function router:next()
  ...
  if next_index > total then next_index = 1 end
  self:set(next_index)
end
...
function router:prev()
  ...
  if prev_index < 1 then prev_index = total end
  self:set(prev_index)
end
...
local function prepare_render_target(self, mon, width, height)
  ...
  if type(window) ~= "table" or type(window.create) ~= "function"
      or type(width) ~= "number" or type(height) ~= "number" or width < 1 or height < 1 then
...
local function build_snapshot(page_name, model)
  local payload
  if model and model.snapshot ~= nil then payload = { page = page_name or "", snapshot = model.snapshot }
  else payload = { page = page_name or "", model = model or {} } end
...
function router:render(mon, model)
  ...
  if page and page.render then
    local ok, result = xpcall(function()
      return page.render(mon, model, should_clear)
    end, function(err)
    ...
    if ok then
      ...
      self.last_error = {
        page = tostring(page.name or "?"), code = "RENDER_FAILED",
        message = tostring(result), ts = os.epoch and os.epoch("utc") or nil,
      }
  ...
  if not w or not h then publish_render_target(mon, buffered); return end
