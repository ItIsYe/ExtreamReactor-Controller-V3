...
assert(visible_calls[1] == true,
  'a newly-created invisible Window needs only one publish call')

r:render(physical, { snapshot = 'two' })
assert(created == 1 and renders == 2, 'normal update must reuse existing Window')
assert(visible_calls[2] == false and visible_calls[3] == true,
  'reused Window must hide while drawing and publish exactly once')
