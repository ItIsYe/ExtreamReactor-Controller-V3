...
local opts={
  controller=controller,target=target,node_id='VALVE-1',label='VALVE-1',
...
assert(last_button.x1 == 5 and last_button.x2 == 46 and last_button.y == 12 and last_button.y2 == 14)
assert(compact:get_touch_geometry().ui_scale == 0.5)
assert(compact:handle_event({'mouse_click',1,25,13}) == true)
assert(#calls == 1 and calls[1].high == true and calls[1].force == true)
assert(compact:handle_event({'mouse_click',1,4,13}) == false)
assert(compact:handle_event({'mouse_click',1,47,13}) == false)
assert(compact:handle_event({'mouse_click',1,25,11}) == false)
assert(compact:handle_event({'mouse_click',1,25,15}) == false)
