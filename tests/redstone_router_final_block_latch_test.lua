...
local r=rr.new({config={logistics={redstone_tree={{integrator='V1',reactor='R1'},{integrator='V2',reactor='R2'}}}},
  comms={get_peers=function() return peers end},log=function() end,warn_once=function() end})
r:refresh()
local function ack(id,applied,high)
...
ack('V1',true,true); ack('V2',true,true); r:tick(clock)
ack('V1',true,false); r:tick(clock); clock=clock+500; r:tick(clock)
...
ack('V1',false,true); ack('V2',true,true); r:tick(clock)
