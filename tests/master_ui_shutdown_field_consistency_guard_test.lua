...
-- Secondary guard: keep token checks for field projection presence.
...
for _, field in ipairs(ui_fields) do
  if not ui_content:find(field, 1, true) then
    error('missing RT shutdown ui field projection token: ' .. tostring(field))
  end
end

-- Primary guard: semantic projection checks for success/cancelled/failed.
local captured_rt
local nodes = {
  ['RT-1'] = {
...
if cancelled.shutdown_state_reached_at ~= nil then error('CANCELLED must not expose shutdown_state_reached_at') end
