...
if not source:find('poll_spent_ms', 1, true) then
  error('expected cumulative matrix polling spent time accounting')
end
if not source:find('poll_spent_ms >= metric_time_budget_ms', 1, true) then
  error('expected matrix polling loop to stop once cumulative time budget is exhausted')
end
if not source:find('run_heartbeat_pump(now_ms())', 1, true) then
  error('expected heartbeat pump invocation while matrix polling runs')
end
if not source:find('time_budget_ms=%d spent_ms=%d', 1, true) then
