...
local router=lib.new({node_id='node-77',config={node_id='ROLE-DEFAULT',logistics={redstone_tree={{reactor='R1',path={'VALVE-1'}}}}},comms={get_peers=function() return {['VALVE-1']={down=false}} end},log=function() end,warn_once=function() end})
router:refresh(); assert(#tx>=1); local sent=tx[#tx]; assert(sent.src=='node-77'); assert(sent.command_id:match('^node%-77%-'))
local key='VALVE-1'; local pending=assert(router._state.pending_valve_acks[key]); assert(pending.src=='node-77')
...
local ok,cid=router:_set_valve('VALVE-1',false); assert(ok and cid); local retry=assert(router._state.pending_valve_acks[key]); retry.sent_ts=-4000; router:check_pending_acks(); local resent=tx[#tx]; assert(resent.command_id==cid and resent.src=='node-77')
