...
ENERGY now models induction data matrix-zentriert (logical matrix objects) and groups multiple `inductionPort_*` access points into one matrix snapshot group with one representative read port, so matrix-wide values (`stored/capacity/cells/providers`) are not redundantly polled per port.
ENERGY matrix polling keeps per-matrix/per-metric timing history and applies adaptive cadence/backoff for outlier calls, while also enforcing a per-matrix poll budget so one slow reader port cannot consume the entire matrix poll budget in a single payload tick.
ENERGY splits matrix snapshots into dynamic cadence (`stored/capacity/input/output`) and static cadence (`cells/providers/ports`) caches; TELEMETRY and UI both read these shared snapshots only, with no direct heavy peripheral calls inside render/send paths.
