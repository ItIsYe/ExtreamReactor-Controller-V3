...
Badge rows (the small colored status labels like `RT OK | M OK | MASTER | CAP | SHD`) are built centrally through `layout.badge_row()`, which knows the monitor width in advance and degrades gracefully: full labels if there's room, short forms if not, lowest-priority badges dropped last if it still doesn't fit. This replaced ad-hoc per-view badge rendering that could silently overlap or get cut off on narrow monitors — a recurring problem before this was introduced 2026-07-08.
