...
ENERGY matrix polling additionally enforces a cumulative per-payload time budget (`matrix_metric_time_budget_ms`) and keeps heartbeat pumping active between slow matrix calls, so communication liveness remains stable even when matrix peripherals respond slowly.
