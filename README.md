...
- **Bootstrap-Aufgabe**: Setzt **einheitlich** `package.path` (inkl. `/xreactor/?.lua`, `/xreactor/?/?.lua`, `/xreactor/?/init.lua`), damit `require("core.*")`, `require("shared.*")`, `require("master.*")` und `require("nodes.*")` immer funktionieren.
